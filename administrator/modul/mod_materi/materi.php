...
switch($_GET[act]){
  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
        ...
        $tampil_materi = mysqli_query($db,"SELECT * FROM file_materi ORDER BY id_kelas");
    ...
    while ($r=mysqli_fetch_array($tampil_materi)){
      $tgl_posting   = tgl_indo($r[tgl_posting]);
       echo "<tr><td>$no</td>
             ...
             $pengajar2 = mysqli_query($db,"SELECT * FROM pengajar WHERE id_pengajar = '$p2[id_pengajar]'");
 ...
 elseif ($_SESSION[leveluser]=='pengajar'){        
    ...
    $cek_materi = mysqli_query($db,"SELECT * FROM file_materi WHERE pembuat = '$_SESSION[idpengajar]'");
    ...
    echo"</table>";   
        ...
        $ambil_siswa = mysqli_query($db,"SELECT * FROM siswa WHERE id_siswa = '$_SESSION[idsiswa]'");
...
case "daftarmateri":
    if ($_SESSION[leveluser] == 'siswa'){
        ...
        $materi = mysqli_query($db,"SELECT * FROM file_materi WHERE id_matapelajaran = '$_GET[id]' LIMIT $posisi,$batas ");
        ...
        echo "</table>";
        $jmldata=mysqli_num_rows(mysqli_query($db,"SELECT * FROM file_materi WHERE id_matapelajaran = '$_GET[id]'"));
...
case "tambahmateri":
    ...
    <dt><label>Kelas</label></dt>              <dd><select name='id_kelas' onChange='showpel()'>
                                          ...
                                          $cari_kelas = mysqli_query($db,"SELECT * FROM kelas ORDER BY nama");
...
case "editmateri":
    if ($_SESSION[leveluser]=='admin'){
    $edit=mysqli_query($db,"SELECT * FROM file_materi WHERE id_file = '$_GET[id]'");
    $m=mysqli_fetch_array($edit);
    $isikelas = mysqli_query($db,"SELECT * FROM kelas WHERE id_kelas = '$m[id_kelas]'");
    $k=mysqli_fetch_array($isikelas);
    $pelajaran = mysqli_query($db,"SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$m[id_matapelajaran]'");
