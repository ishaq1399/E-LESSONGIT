...
$cek_siswa = mysqli_query($db,"SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq='$_POST[id]' AND id_siswa='$_SESSION[idsiswa]'");
$info_siswa = mysqli_fetch_array($cek_siswa);
if ($info_siswa[hits]<= 0){
    mysqli_query($db,"INSERT INTO siswa_sudah_mengerjakan (id_tq,id_siswa,hits)
                                        VALUES ('$_POST[id]','$_SESSION[idsiswa]',hits+1)");
}
elseif ($info_siswa[hits] > 0){
}

$soal = mysqli_query($db,"SELECT * FROM quiz_pilganda where id_tq='$_POST[id]' ORDER BY rand()");
$pilganda = mysqli_num_rows($soal);
$soal_esay = mysqli_query($db,"SELECT * FROM quiz_esay WHERE id_tq='$_POST[id]'");
