  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
     ...
     $posisi = $p->cariPosisi($batas);
      ...
      $tampil_siswa = mysqli_query($db,"SELECT * FROM siswa ORDER BY id_kelas LIMIT $posisi,$batas");
    ...
    echo "</table>";
    $jmldata=mysqli_num_rows(mysqli_query($db,"SELECT * FROM siswa"));
...
case "lihatmurid":
    ...
    $tampil = mysqli_query($db,"SELECT * FROM siswa WHERE id_kelas = '$_GET[id]' ORDER BY nama_lengkap LIMIT $posisi,$batas");
...
case "tambahsiswa":
    if ($_SESSION[leveluser]=='admin'){
        $tampil = mysqli_query($db,"SELECT * FROM siswa WHERE id_siswa = '$_GET[id]'");
        ...
        echo "
          ...
          <dt><label>Kelas</label></dt>        <dd> : <select name='id_kelas'>
                                           ...
                                           $tampil=mysqli_query($db,"SELECT * FROM kelas ORDER BY id_kelas");
                                           while($r=mysqli_fetch_array($tampil)){
  ...
  case "editsiswa":
    $edit=mysqli_query($db,"SELECT * FROM siswa WHERE id_siswa='$_GET[id]'");
    $r=mysqli_fetch_array($edit);
    $get_kelas = mysqli_query($db,"SELECT * FROM kelas WHERE id_kelas = '$r[id_kelas]'");
    ...
    echo "<form method=POST action=$aksi?module=siswa&act=update_siswa enctype='multipart/form-data'>
          ...
          <dt><label>Kelas</label></dt>        <dd> : <select name='id_kelas'>
                                           ...
                                           $tampil=mysqli_query($db,"SELECT * FROM kelas ORDER BY nama");
 ...
 case "detailsiswa":
    if ($_SESSION[leveluser]=='admin'){
       $detail=mysqli_query($db,"SELECT * FROM siswa WHERE id_siswa='$_GET[id]'");
       $siswa=mysqli_fetch_array($detail);
       $tgl_lahir   = tgl_indo($siswa[tgl_lahir]);

       $get_kelas = mysqli_query($db,"SELECT * FROM kelas WHERE id_kelas = '$siswa[id_kelas]'");
       $kelas = mysqli_fetch_array($get_kelas);
