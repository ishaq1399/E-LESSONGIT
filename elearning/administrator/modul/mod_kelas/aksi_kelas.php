...
if ($module=='kelas' AND $act=='input_kelas'){
  mysqli_query($db,"INSERT INTO kelas(id_kelas,
                                 nama,
                                 id_pengajar,
                                 id_siswa)
...
elseif ($module=='kelas' AND $act=='hapuswalikelas'){
  $kelas = mysqli_query($db,"SELECT * FROM siswa WHERE id_kelas = '$_GET[id]'");
  $r = mysqli_fetch_array($kelas);

  mysqli_query($db,"UPDATE siswa SET jabatan = 'Siswa'
                                WHERE id_siswa = '$r[id_siswa]'");
  mysqli_query($db,"UPDATE kelas SET id_pengajar  = '0',
                                id_siswa  = '0'
                        WHERE id = '$_GET[id]'");
  header('location:../../media_admin.php?module='.$module);
}
...
elseif ($module=='kelas' AND $act=='update_kelas'){
  mysqli_query($db,"UPDATE kelas SET id_kelas = '$_POST[id_kelas]',
                                nama = '$_POST[nama]',
...
elseif ($module=='kelas' AND $act=='input_walikelas'){
  $cari = mysqli_query($db,"SELECT * FROM kelas WHERE id_kelas = '$_POST[kelas]'");
  $r = mysqli_fetch_array($cari);
  mysqli_query($db,"UPDATE kelas SET id_pengajar  = '$_SESSION[idpengajar]',
                                id_siswa  = '$_POST[ketua]'
                        WHERE id = '$r[id]'");
  mysqli_query($db,"UPDATE siswa SET jabatan = 'Ketua Kelas'
                                WHERE id_siswa = '$_POST[ketua]'");
  header('location:../../media_admin.php?module=home');
}

elseif ($module=='kelas' AND $act=='update_walikelas'){
  $cek = mysqli_query($db,"SELECT * FROM kelas WHERE id = '$_POST[id]'");
  $c = mysqli_fetch_array($cek);
  $cek_siswa = mysqli_query($db,"SELECT id_siswa FROM kelas WHERE id = '$_POST[id]'");
  ...
  if ($_POST['kelas']==$c[id_kelas]){
    ...
    if(!empty($s)){
         ...
         mysqli_query($db,"UPDATE kelas SET id_siswa  = '$_POST[ketua]'
                        WHERE id = '$_POST[id]'");
         mysqli_query($db,"UPDATE siswa SET jabatan = 'Ketua Kelas'
                                WHERE id_siswa = '$_POST[ketua]'");
    }else{
...
elseif ($module=='kelas' AND $act=='update_kelas_siswa'){
    mysqli_query($db,"UPDATE siswa SET id_kelas         = '$_POST[id_kelas]'
