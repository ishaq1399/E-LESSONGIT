...
$login=mysqli_query($db,"SELECT * FROM admin WHERE username='$username' AND password='$pass' AND blokir='N'");
$ketemu=mysqli_num_rows($login);
$r=mysqli_fetch_array($login);

$login_guru =mysqli_query($db,"SELECT * FROM pengajar WHERE username_login='$username' AND password_login='$pass' AND blokir='N'");
...
if ($ketemu > 0){
  ...
  mysqli_query($db,"UPDATE admin SET id_session='$sid_baru' WHERE username='$username'");
...
elseif($ketemu_guru > 0){
  ...
  mysqli_query($db,"UPDATE pengajar SET id_session='$sid_baru' WHERE username_login='$username'");
  header('location:media_admin.php?module=home');
}
else{
   echo "<link href=css/style.css rel=stylesheet type=text/css>";
